...
enum GiantFishState {
	...
	DIVE = BossState.MAX_BOSS_STATES,
	SUBMERGED_MOVE,
	EMERGE_JUMP,
	# 擺尾攻擊狀態
	TAIL_SWIPE, 
...
func take_damage(damage: float, attacker: Node, knockback_info: Dictionary = {}) -> void:
	if current_state == GiantFishState.APPEAR:
		return

	super.take_damage(damage, attacker, knockback_info)
...
func _change_state(new_state: int) -> void:
	...
	var old_state_name = GiantFishState.keys()[GiantFishState.values().find(current_state)]
	var new_state_name = GiantFishState.keys()[GiantFishState.values().find(new_state)]
...
func _on_animation_finished() -> void:
	if current_state == BossState.HURT and is_counter_attack_pending:
...
func _is_state_an_attack(state: int) -> bool:
	...
	return super._is_state_an_attack(state)
