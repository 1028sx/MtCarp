...
func set_touch_damage_active(is_active: bool) -> void:
	if not is_instance_valid(touch_damage_area):
		printerr("[BossBase] TouchDamageArea is not valid!")
		return
	...
	if is_instance_valid(collision_shape):
		...
		printerr("[BossBase] Could not find CollisionShape2D in TouchDamageArea!")
...
func _on_hitbox_area_entered(area: Area2D) -> void:
	if area.is_in_group("player_attack"):
		var damage_amount = area.get_parent().get("damage") if area.get_parent().has_method("get") else 10
